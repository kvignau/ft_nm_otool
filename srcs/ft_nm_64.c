...
static int	check_lc_corrupt_64(void *ptr, size_t buf_size)
{
	int						i;
	int						ncmds;
	size_t					acc;
	...
	while (i++ < ncmds)
	{
		if (tmp->cmd == LC_SEGMENT_64)
			if (check_seg_corrupt_64((struct segment_command_64 *)tmp, buf_size))
				return (EXIT_FAILURE);
		acc += tmp->cmdsize;
...
int		ft_handle_64(void *ptr, char **sections, size_t buf_size)
{
	...
	while (i++ < ncmds)
	{
		if (lc->cmd == LC_SEGMENT_64)
			sections = ft_get_section_64(sections,
				(struct segment_command_64 *)lc);
		if (lc->cmd == LC_SYMTAB)
		{
			sym = (struct symtab_command *)lc;
			get_sym_64(sym, ptr, sections);
		}
